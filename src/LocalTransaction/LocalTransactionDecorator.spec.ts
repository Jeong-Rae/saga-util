...
class TestService {
    ...
    async successMethod(): Promise<string> {
        const result = await withRollback(Promise.resolve('fn'))
            .rollback(() => {
    ...
    async errorMethod(): Promise<void> {
        await withRollback(Promise.resolve('fn1'))
            .rollback(() => this.rollbackCalls.push('rollbackFn1'));
        await withRollback(Promise.resolve('fn2'))
            .rollback(() => this.rollbackCalls.push('rollbackFn2'));
        throw new Error('Test error');
    }

    @LocalTransaction()
    async nestedMethod(): Promise<void> {
        await withRollback(Promise.resolve('outerFn'))
            .rollback(() => this.rollbackCalls.push('outerRollbackFn'));
        await this.innerMethod();
    }

    @LocalTransaction()
    async innerMethod(): Promise<void> {
        await withRollback(Promise.resolve('innerFn'))
            .rollback(() => this.rollbackCalls.push('innerRollbackFn'));
...
describe('LocalTransactionDecorator', () => {
    ...
    it('컨텍스가 중첩 될 경우에도 롤백 컨텍스트 보장', async () => {
        ...
        expect(service.rollbackCalls).toEqual(['innerRollbackFn', 'outerRollbackFn']);
    });

    describe('propagation', () => {
        it('propagation: new', async () => {
            await expect(service.nestedMethodNewContext()).rejects.toThrow('Inner error');

            expect(service.parentContext).toBeDefined();
            expect(service.childContext).toBeDefined();

            expect(service.childContext?.getContextId()).not.toBe(service.parentContext?.getContextId());
