...
describe('withRollback', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(localTransactionContextStorage, 'getStore')
    ...
    describe('rollbackFn이 정상적으로 등록됨', () => {
        it('동기 rollbackFn 등록', async () => {
            ...
            const usage = await withRollback(Promise.resolve('fn'))
                .rollback(syncRollbackFn);
        ...
        it('비동기 rollbackFn 등록', async () => {
            ...
            await withRollback(Promise.resolve('fn'))
                .rollback(asyncRollbackFn);
